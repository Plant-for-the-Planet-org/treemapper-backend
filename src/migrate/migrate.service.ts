  ...
  async startUserMigration(
    ...
    try {
      ...
      if (personalProject.length > 0) {
        ...
        const newPersonalProject = await this.projectService.createPersonalProject(payload, userId, 1)
  ...
  private async migrateUserSpecies(uid: number, authToken: string, migrationId: number, email: string): Promise<boolean> {
    try {
      ...
      if (personalProject.length > 0) {
        ...
        const newPersonalProject = await this.projectService.createPersonalProject(payload, uid, 1)
